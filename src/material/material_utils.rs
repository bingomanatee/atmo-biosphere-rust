...
impl MaterialUtils {
    /// Convert gas interference factor from scaled u32 back to original f64 (0.0-1.0 range)
    ...
    /// Convert thermal conduction modifier from scaled u32 back to original f64
    ...
    /// Convert thermal expansivity from scaled u32 back to original f64 (very small values like 1e-05)
    pub fn thermal_expansivity_as_f64(phase: &MaterialPhase) -> Option<f64> {
        phase.thermal_expansivity.map(|val| val as f64 / 1e9)
    }

    /// Convert activation volume from scaled u32 back to original f64 (very small values like 1e-05)
    ...
    /// Convert dynamic viscosity from u64 to f64 (can be very large values like 1e+25)
    pub fn dynamic_viscosity_as_f64(phase: &MaterialPhase) -> Option<f64> {
        phase.dynamic_viscosity.map(|val| val as f64)
    }

    /// Convert bulk modulus from u64 to f64 (large values like 130000000000)
    ...
    pub fn melt_temp_as_f64(phase: &MaterialPhase) -> Option<f64> {
        phase.melt_temp.map(|val| val as f64)
    }
    ...
    pub fn boil_temp_as_f64(phase: &MaterialPhase) -> Option<f64> {
        phase.boil_temp.map(|val| val as f64)
    }

    /// Get latent heat of fusion as f64 for calculations
    pub fn latent_heat_fusion_as_f64(phase: &MaterialPhase) -> Option<f64> {
        phase.latent_heat_fusion.map(|val| val as f64)
    }

    /// Get latent heat of vaporization as f64 for calculations
    pub fn latent_heat_vapor_as_f64(phase: &MaterialPhase) -> Option<f64> {
        phase.latent_heat_vapor.map(|val| val as f64)
    }
    ...
    pub fn print_phase_properties(phase: &MaterialPhase, phase_name: &str) {
        ...
        if let Some(temp) = phase.melt_temp {
            println!("Melting temperature: {} K", temp);
        }
        if let Some((min, max)) = Self::melt_temp_range_as_f64(phase) {
            println!("Melting temperature range: {:.2} - {:.2} K", min, max);
        }
        if let Some(temp) = phase.boil_temp {
            println!("Boiling temperature: {} K", temp);
        }
        if let Some(heat) = phase.latent_heat_fusion {
            println!("Latent heat of fusion: {} J/kg", heat);
        }
        if let Some(heat) = phase.latent_heat_vapor {
            println!("Latent heat of vaporization: {} J/kg", heat);
        }
