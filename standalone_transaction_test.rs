// Completely standalone transaction manager test
// This tests the transaction management concepts without any dependencies
...
impl SimpleTransactionManager {
    ...
    fn validate_cell_transactions(
        ...
        println!("   Debug: Cell {} - Mass limit: {:.2e}, Energy limit: {:.2e}",
            baseline.cell_id, max_mass_change, max_energy_change);
        println!("   Debug: Proposed - Mass: {:.2e}, Energy: {:.2e}",
            total_mass_delta, total_energy_delta);
        ...
        println!("   Debug: Violation factors - Mass: {:.2}, Energy: {:.2}, Max: {:.2}",
            mass_violation_factor, energy_violation_factor, max_violation_factor);

        if max_violation_factor > 1.0 {
            let scaling_factor = 1.0 / max_violation_factor;
            println!("   Debug: Scaling required - factor: {:.3}", scaling_factor);
            (false, scaling_factor, format!("Mass: {:.1}x limit, Energy: {:.1}x limit",
                mass_violation_factor, energy_violation_factor))
        } else {
            println!("   Debug: Within limits - no scaling needed");
...
fn main() {
    ...
    println!("Testing transaction management with geological timescale limits:");
    println!("  - Mass transfer: 0.1% per year maximum");
    println!("  - Energy transfer: 0.5% per year maximum");
    ...
    test_identify_problematic_mechanisms();
    
    println!("\n✅ All Standalone Transaction Tests Passed!");
    println!("\n🎯 Key Findings:");
    println!("   - Transaction system successfully regulates excessive transfers");
    println!("   - Geological timescale limits (0.5% energy, 0.1% mass) are enforced");
    println!("   - Multiple competing mechanisms are properly coordinated");
    println!("   - Mass conservation is maintained through double-entry accounting");
    println!("   - Problematic mechanisms can be identified by transaction source");
}
...
fn test_basic_functionality() {
    ...
    println!("   ✅ Transaction manager created with geological timescale limits");
}

fn test_reasonable_transactions() {
    println!("\n🔬 Test: Reasonable Transactions (Within Geological Limits)");
    
    let mut tm = SimpleTransactionManager::new();
    
    // Create baseline cell (typical mantle cell)
    ...
    // Small reasonable transactions (well within geological limits)
    tm.propose_transaction(Transaction {
        source: TransactionSource::ThermalConduction,
        source_cell_id: 1,
        target_cell_id: Some(2),
        energy_delta_joules: -cell_a.energy_joules * 0.001, // 0.1% energy
        mass_delta_kg: -cell_a.mass_kg * 0.0001,           // 0.01% mass
        description: "Small thermal conduction".to_string(),
    });

    let regulated = tm.validate_and_regulate_transactions(100.0); // 100 years (more realistic)
    assert_eq!(regulated.len(), 1);
    assert!(!regulated[0].description.contains("SCALED"));
    
    tm.commit_transactions(regulated);
    println!("   ✅ Reasonable geological transfers processed without scaling");
}

fn test_excessive_transactions() {
    println!("\n🔬 Test: Excessive Transactions (Beyond Geological Limits)");
    ...
    // Excessive transaction (unrealistic for geological timescales)
    tm.propose_transaction(Transaction {
        source: TransactionSource::ConvectionPlume,
        source_cell_id: 1,
        target_cell_id: Some(2),
        energy_delta_joules: -cell_a.energy_joules * 0.6,  // 60% energy (over limit)
        mass_delta_kg: -cell_a.mass_kg * 0.12,             // 12% mass (over limit)
    ...
    let scaled = &regulated[0];
    assert!(scaled.energy_delta_joules.abs() < cell_a.energy_joules * 0.6); // Less than original 60%
    assert!(scaled.mass_delta_kg.abs() < cell_a.mass_kg * 0.12);           // Less than original 12%
    
    tm.commit_transactions(regulated);
    println!("   ✅ Excessive geological transfers properly scaled to realistic rates");
}

fn test_multiple_competing_transactions() {
    println!("\n🔬 Test: Multiple Competing Mechanisms");
    ...
    // Multiple geological mechanisms competing for the same cell
    let transactions = vec![
        Transaction {
            source: TransactionSource::ThermalConduction,
            source_cell_id: 1,
            target_cell_id: Some(2),
            energy_delta_joules: -cell_a.energy_joules * 0.3, // 30%
            mass_delta_kg: -cell_a.mass_kg * 0.03,           // 3%
            description: "Thermal conduction".to_string(),
        },
        Transaction {
            source: TransactionSource::ConvectionPlume,
            source_cell_id: 1,
            ...
            energy_delta_joules: -cell_a.energy_joules * 0.3, // 30%
            mass_delta_kg: -cell_a.mass_kg * 0.03,           // 3%
            description: "Convection plume".to_string(),
        },
        Transaction {
            source: TransactionSource::SurfaceCooling,
            source_cell_id: 1,
            target_cell_id: None,
            energy_delta_joules: -cell_a.energy_joules * 0.2, // 20%
            mass_delta_kg: 0.0,
    ...
    // Total: 80% energy, 6% mass - way exceeds geological limits
    let regulated = tm.validate_and_regulate_transactions(100.0); // 100 years
    ...
    tm.commit_transactions(regulated);
    println!("   ✅ Multiple competing mechanisms coordinated ({} scaled to geological rates)", scaled_count);
}

fn test_mass_conservation() {
    println!("\n🔬 Test: Mass Conservation (Double-Entry Accounting)");
    ...
    let mass_transfer = cell_a.mass_kg * 0.0001; // 0.01% transfer

    // Balanced mass transfer (what goes out must come in)
    tm.propose_transaction(Transaction {
        source: TransactionSource::ConvectionPlume,
        source_cell_id: 1,
        target_cell_id: Some(2),
        energy_delta_joules: 0.0,
        mass_delta_kg: -mass_transfer, // Remove from source
        description: "Mass transfer out".to_string(),
    });
    ...
    tm.propose_transaction(Transaction {
        ...
        target_cell_id: None,
        energy_delta_joules: 0.0,
        mass_delta_kg: mass_transfer, // Add to target
        description: "Mass transfer in".to_string(),
    });

    let regulated = tm.validate_and_regulate_transactions(100.0); // 100 years
    assert_eq!(regulated.len(), 2);
    
    // Verify net mass change is zero (conservation)
    let total_mass_delta: f64 = regulated.iter().map(|t| t.mass_delta_kg).sum();
    assert!(total_mass_delta.abs() < 1e-10);
    
    tm.commit_transactions(regulated);
    println!("   ✅ Mass conservation verified (net change: {:.2e} kg)", total_mass_delta);
}

fn test_energy_limits_by_timescale() {
    println!("\n🔬 Test: Energy Limits Scale with Geological Time");
    ...
    // Test different geological timescales
    let test_cases = vec![
        (1000.0, 0.005),   // 1,000 years: 0.5% allowed
        (10000.0, 0.05),   // 10,000 years: 5% allowed  
        (100000.0, 0.5),   // 100,000 years: 50% allowed
    ];
    ...
    for (years_per_step, expected_max_fraction) in test_cases {
        ...
        tm.propose_transaction(Transaction {
            ...
            mass_delta_kg: 0.0,
            description: "At geological limit".to_string(),
        });
        ...
        tm.commit_transactions(regulated);
        println!("   ✅ {:.0} years: {:.1}% energy transfer allowed", years_per_step, expected_max_fraction * 100.0);
    }
}

fn test_identify_problematic_mechanisms() {
    println!("\n🔬 Test: Identify Problematic Mechanisms");
    ...
    // Simulate different mechanisms with varying degrees of excess
    let problematic_transactions = vec![
        Transaction {
            source: TransactionSource::ConvectionPlume,
            source_cell_id: 1,
            target_cell_id: Some(2),
            energy_delta_joules: -cell_a.energy_joules * 0.15, // 15% - very excessive
            mass_delta_kg: -cell_a.mass_kg * 0.02,             // 2% - very excessive
            description: "Runaway plume system".to_string(),
        },
        Transaction {
            source: TransactionSource::ThermalConduction,
            source_cell_id: 1,
            target_cell_id: Some(3),
            energy_delta_joules: -cell_a.energy_joules * 0.001, // 0.1% - reasonable
            mass_delta_kg: 0.0,
            description: "Normal conduction".to_string(),
        },
    ];
    ...
    // Check which mechanisms were scaled (problematic)
    for transaction in &regulated {
        if transaction.description.contains("SCALED") {
            println!("   🚨 PROBLEMATIC: {:?} mechanism exceeded limits", transaction.source);
        } else {
            println!("   ✅ NORMAL: {:?} mechanism within limits", transaction.source);
        }
    }
    ...
    println!("   ✅ Successfully identified which mechanisms exceed geological limits");
}
